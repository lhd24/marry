...
(function($){
    flux.slider = function(elem, opts) {
        ...
        this.imageContainer = $('<div class="images loading1"></div>').css({
            'position': 'relative',
            'overflow': 'hidden'
        ...
        if(this.options.autoplay&&(typeof(autoSta)=='undefined'||autoSta!=1))
            this.start();
        ...
        this.element.bind('mousedown touchstart', function(event) {
            if(event.type == 'touchstart'){
                startPoint.left = event.originalEvent.touches[0].pageX;
            } else if(event.type == 'mousedown') {
                startPoint.left = event.pageX;
            }
        }).bind('mouseup touchend', function(event) {
            if(event.type == 'touchend'){
                endPoint.left = event.originalEvent.changedTouches[0].pageX;
    ...
    flux.slider.prototype = {
        ...
        start: function() {
            ...
            if(typeof(this.options.onStartEnd) == 'function') {
        ...
        finishedLoading: function() {
            ...
            // Should we setup a pagination view?
				console.log(this.pagination,this.options.pagination);
        ...
        setupImages: function() {
            var img1 = this.getImage(this.currentImageIndex),
                css1 = {
                    'background': 'url("'+img1.src+'") 50% 50% / contain no-repeat ' + this.options.bgColor,
                    zIndex: 101,
                    'cursor': 'auto'
                };
...
(function($) { 
	flux.transitions.flash = function(fluxslider, opts) {
		return new flux.transition(fluxslider, $.extend({
			direction: "left",
			setup: function() {
				var b = this.slider.image1.width(),
					c = this.slider.image1.height(),
					d = $('<div class="current"></div>').css({
						height: c + "px",
						width: b + "px",
						position: "absolute",
						top: "0px",
						left: "0px",
						background: this.slider["left" == this.options.direction ? "image2" : "image2"].css("background")
					}).css3({
						"backface-visibility": "hidden"
					}),
					e = $('<div class="next"></div>').css({
						height: c + "px",
						width: b + "px",
						position: "absolute",
						top: "0px",
						left: "0px",
						background: this.slider["left" == this.options.direction ? "image1" : "image1"].css("background")
					}).css3({
						"backface-visibility": "hidden"
					});
				this.slideContainer = $('<div class="slide"></div>').css({
					...
					height: c + "px",
					position: "relative",
					left: "left" == this.options.direction ? "0px" : "0px",
					"z-index": 101
				}).css3({
